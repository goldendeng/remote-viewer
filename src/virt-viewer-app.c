...
virt_viewer_app_default_activate(VirtViewerApp *self, GError **error)
{
    ...
    if (fd >= 0) {
        ...
        return virt_viewer_session_open_host(VIRT_VIEWER_SESSION(priv->session),
                                             priv->ghost, priv->gport, priv->gtlsport, priv->gpasswd);
    } else {
...
virt_viewer_app_activate(VirtViewerApp *self, GError **error)
{
    VirtViewerAppPrivate *priv;
    gboolean ret;
    g_debug("virt_viewer_app_activate");
    g_return_val_if_fail(VIRT_VIEWER_IS_APP(self), FALSE);
                              ...
                              GValue *value G_GNUC_UNUSED, GParamSpec *pspec)
{
    ...
    case PROP_GHOST:
        g_value_set_string(value, priv->ghost);  
        break;

    case PROP_GPORT:
        g_value_set_string(value, priv->gport);	         
        break;

    case PROP_GTLSPORT:
	g_value_set_string(value, priv->gtlsport);
        break;

    case PROP_GPASSWD:
	g_value_set_string(value, priv->gpasswd);
        break;
                              ...
                              const GValue *value G_GNUC_UNUSED, GParamSpec *pspec)
{
    ...
    case PROP_GHOST:
        g_free(priv->ghost);
        priv->ghost = g_value_dup_string(value);
        //rt_viewer_app_update_pretty_address(self);
        break;

    case PROP_GPORT:
        g_free(priv->gport);
        priv->gport = g_value_dup_string(value);
        //rt_viewer_app_update_pretty_address(self);
        break;

    case PROP_GTLSPORT:
        g_free(priv->gtlsport);
        priv->gtlsport = g_value_dup_string(value);
        //rt_viewer_app_update_pretty_address(self);
        break;

    case PROP_GPASSWD:
        g_free(priv->gpasswd);
        priv->gpasswd = g_value_dup_string(value);
        //rt_viewer_app_update_pretty_address(self);
        break;
                             ...
                             GObjectConstructParam *properties)
{
    ...
    gtk_accel_map_add_entry("<virt-viewer>/view/toggle-fullscreen", GDK_F12, GDK_SHIFT_MASK);
    gtk_accel_map_add_entry("<virt-viewer>/view/release-cursor", GDK_F11, GDK_SHIFT_MASK);
...
virt_viewer_app_class_init (VirtViewerAppClass *klass)
{
    ...
    g_object_class_install_property(object_class,
     ...
     g_object_class_install_property(object_class,
                                    PROP_GHOST,
                                    g_param_spec_string("ghost",
                                                        "gport",
                                                        "Remote graphical host",
                                                        "",
                                                        G_PARAM_READWRITE |
                                                        G_PARAM_STATIC_STRINGS));
     ...
     g_object_class_install_property(object_class,
                                    PROP_GPORT,
                                    g_param_spec_string("gport",
                                                        "gport",
                                                        "Remote graphical port",
                                                        "",
                                                        G_PARAM_READWRITE |
                                                        G_PARAM_STATIC_STRINGS));
     ...
     g_object_class_install_property(object_class,
                                    PROP_GTLSPORT,
                                    g_param_spec_string("gtlsport",
                                                        "gtlsport",
                                                        "Remote graphical tlsport",
                                                        "",
                                                        G_PARAM_READWRITE |
                                                        G_PARAM_STATIC_STRINGS));
     ...
     g_object_class_install_property(object_class,
                                    PROP_GPASSWD,
                                    g_param_spec_string("gpasswd",
                                                        "gpasswd",
                                                        "Remote graphical password",
                                                        "",
                                                        G_PARAM_READWRITE |
                                                        G_PARAM_STATIC_STRINGS));
