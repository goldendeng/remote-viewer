...
RemoteViewer *
remote_viewer_new(const gchar *uri, const gchar *host, const gchar *port, const gchar *tlsport, const gchar *passwd)
{
    return g_object_new(REMOTE_VIEWER_TYPE,
                        ...
                        "gtlsport", tlsport,
                        "gpasswd", passwd,
                        "open-recent-dialog", uri == NULL&&host == NULL,
                        NULL);
}
...
connect_dialog(GtkWindow *main_window, gchar **uri)
{
    ...
    gtk_entry_set_activates_default(GTK_ENTRY(entry), TRUE);//dian wan zhe ge jiu guan bi dui hua kuang!!
...
retry_dialog:
    ...
    ret = VIRT_VIEWER_APP_CLASS(remote_viewer_parent_class)->start(app);//display
