...
static gboolean virt_viewer_session_spice_open_host(VirtViewerSession *session, const gchar *host, const gchar *port, const gchar *tlsport, const gchar *password);
...
create_spice_session(VirtViewerSessionSpice *self)
{
    ...
    virt_viewer_signal_connect_object(self->priv->session, "channel-new",
                                      G_CALLBACK(virt_viewer_session_spice_channel_new), self, 0);//spice-gtk-session (channel_new) g_signal_emit
...
static void
virt_viewer_session_spice_input_channel_event(SpiceChannel *channel G_GNUC_UNUSED,
                                             SpiceChannelEvent event,
                                             VirtViewerSession *session)
{
    VirtViewerSessionSpice *self = VIRT_VIEWER_SESSION_SPICE(session);
   ...
   // static gboolean username_required = FALSE;
    ...
    case SPICE_CHANNEL_ERROR_INPUT:
        ...
        if (self->priv->session)
            spice_session_disconnect(self->priv->session);
	g_signal_emit_by_name(session, "session-reconnect");
        break;
    default:
        break;
    	}
